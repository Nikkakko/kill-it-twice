...
- **v5:** A post-v4 review pass, independent of any failing gate, found four gaps against the reliability rules this spec already states and corrected all four. Sink retry was a flat 1,000ms interval; the reliability rules require retries to be bounded and backed off, so the worker now backs off exponentially per sink (1s doubling to a 30s cap), reset on success. The independent consumer could `nack`-requeue a malformed message forever, which is an unbounded retry on a deterministically unfixable input; parse failures are now routed once to a dedicated `replication.consumer.dlq` queue instead of being retried, while genuinely transient database-write failures still retry indefinitely with the same bounded backoff. The UI's simulation panel did not expose a control for generating a single source change, even though the API already supported it and tech.md's brief requires it; the control was added. Elasticsearch deletes were hard deletes, so a later out-of-order upsert carrying an older version had no document left to lose a version check against and could resurrect a deleted record; deletes now write a version-guarded tombstone document instead, and search results filter tombstones out. No gate's pass/fail behavior changed; `make verify` was re-run clean after each fix.
